...
def coerce_popup_status(status: dict[str, Any], action: str = 'updated') -> dict[str, Any]:
    return {
        ...
        'tab_id': _safe_int(status['tabId']) if isinstance(status.get('tabId'), int) else None,
        ...
        'text_length': _safe_int(status.get('textLength', 0)),
        'action': action,
...
def coerce_popup_message(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        'tab_id': _safe_int(payload['tabId']) if isinstance(payload.get('tabId'), int) else None,
...
def coerce_screen_share_status(status: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        'viewer_window_id': _safe_int(status['viewerWindowId']) if isinstance(status.get('viewerWindowId'), int) else None,
...
def coerce_file_transfer_status(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        'byte_count': _safe_int(payload.get('byteCount', 0)),
...
def build_binary_envelope(metadata: dict[str, Any], payload_bytes: bytes) -> bytes:
    ...
    if len(metadata_bytes) > MAX_BINARY_METADATA_BYTES:
        raise ValueError('Binary envelope metadata exceeds the 1 MiB safety limit.')
...
def parse_binary_envelope(raw_message: bytes) -> tuple[dict[str, Any], bytes] | None:
    ...
    if metadata_length <= 0 or metadata_length > MAX_BINARY_METADATA_BYTES or len(raw_message) < 4 + metadata_length:
        return None
    try:
